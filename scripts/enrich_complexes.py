...
def _make_session() -> requests.Session:
    ...
    if PROXY_URL:
        ...
        s.verify = False
    # Прогрев: посещаем главную страницу города чтобы получить cookies
    try:
        s.get(f'https://{city_slug}.cian.ru/', timeout=12, allow_redirects=True)
    except Exception:
        pass
    return s
...
def _solve_anticaptcha(page_url: str, sitekey: str = None) -> str | None:
    ...
    try:
        ...
        _sitekey = sitekey or _extract_cian_sitekey() or '6LfD3PIbAAAAAJs_eEHvoOl75_83eXSqpPSRFJ_u'
...
def _build_address(geo: dict) -> str | None:
    ...
    wanted = ['street', 'house', 'district', 'city', 'region']
    result = []
    for part in parts:
...
def collect_apartments_all_region(
    ...
    """
    # Первичные ценовые диапазоны (рубли) — охватывают весь спектр новостроек Краснодара/Сочи
    PRICE_SPLITS = [
        (None,        3_500_000),
        (3_500_000,   4_500_000),
        (4_500_000,   5_200_000),
...
def _extract_from_offers(offers: list, jk_data: dict, apt_data: dict):
    ...
    for o in offers:
        ...
        _PLAN_TAGS = {'plan', 'layout', 'flatplan', 'flat-plan', 'planir'}
        plan_img = next((
            p.get('fullUrl') for p in (o.get('photos') or [])
            if p.get('fullUrl') and (p.get('tag') or '').lower() in _PLAN_TAGS
        ), None)
        regular_photos = [
            p.get('fullUrl') for p in (o.get('photos') or [])
            if p.get('fullUrl') and (p.get('tag') or '').lower() not in _PLAN_TAGS
        ]
        ...
        rooms_label = {0: 'Студия', 1: '1-комн', 2: '2-комн', 3: '3-комн',
            ...
            o.get('roomsCount', 0), f'{o.get("roomsCount","?")}к'
        )
...
def collect_all_data_newobjectsale() -> tuple[dict, dict]:
    ...
    for pg in range(1, TOTAL_PAGES + 1):
        ...
        for o in offers:
            ...
            if not d['jk_url']:
                d['jk_url'] = o.get('jkUrl') or o.get('fullUrl')
...
def upsert_apartments(cur, apt_data: dict, rc_map: dict):
    ...
    for cian_id, apt in apt_data.items():
        ...
        # Slug: kv- prefix + cian_id ensures uniqueness per city
        slug = f'kv-{cian_id}'
        ...
        cur.execute("""
            INSERT INTO properties (
                ...
                building_number, complex_building_name, plan_image,
                source_url, url,
        ...
        """, (
            ...
            (apt.get('building_name') or '')[:99] or None,  # complex_building_name mirror
...
def main():
    ...
    if mode == 'prices':
        ...
        for i in range(0, len(apt_items), 500):
            ...
            ins, upd, skip = upsert_apartments(cur, batch, rc_map)
        ...
        # Phase 4d — помечаем проданные (только при достаточном покрытии)
        seen_ids = set(apt_data_raw.keys())
        if seen_ids:
            cur.execute("""
            ...
            if coverage < 0.6:
                ...
                cur.execute("""
                    UPDATE properties SET is_active=FALSE, status='sold', updated_at=NOW()
                    WHERE city_id=%s AND inner_id=ANY(%s::varchar[])
                """, (CITY_ID, list(sold_ids)))
                conn.commit()
                print(f'\n🔴 Помечено как проданные/снятые: {cur.rowcount} квартир')
            else:
                print(f'\n✅ Проданных/снятых не обнаружено')
    ...
    if phase == 1:
        ...
        def _save_1b(apt_d, segs_done):
            ...
            """
            nonlocal _1b_written
            cache['apt_data']         = apt_d
            ...
            try:
                ...
                # ── Upsert apartments ───────────────────────────────────────
                ins, upd, _skip = upsert_apartments(cur, new_apts, rc_map_1b)
                conn.commit()
    ...
    if phase <= 2:
        ...
        for url in pending_urls:
            ...
            print(f'  {url[-40:]:40} | ф:{photos_count:2d} | п:{layouts_count:2d} | д:{has_desc} | '
        ...
        for jk_cian_id, houses in nb_to_houses.items():
            for house_id, expected in houses:
                ...
                if processed % 15 == 0 or processed == total_buildings:
                    ...
                    if new_apts:
                        try:
                            ins, upd, _ = upsert_apartments(cur, new_apts, rc_map_1c)
                            conn.commit()
    ...
    if new_jk_candidates and CREATE_NEW_JK:
        print(f'\n🆕 Новых ЖК (нет в БД): {len(new_jk_candidates)}')
    ...
    # ── 4c. Вставка квартир ───────────────────────────────────────────────────
    print(f'\n🏠 Шаг 4c: Запись квартир в БД ({len(apt_data_raw)} штук)...')
    ...
    for i in range(0, len(apt_items), batch_size):
        batch = dict(apt_items[i:i+batch_size])
        ins, upd, skip = upsert_apartments(cur, batch, rc_map)
        total_inserted += ins
        total_updated  += upd
        total_skipped  += skip
    ...
    # Все inner_id которые мы только что видели на ЦИАН
    seen_inner_ids = set(apt_data_raw.keys())  # строки
    if seen_inner_ids:
        ...
        cur2 = conn2.cursor()
        # Квартиры в нашей БД для Краснодара, которые были active
        cur2.execute("""
            SELECT inner_id FROM properties
            WHERE city_id = %s AND is_active = TRUE AND inner_id IS NOT NULL
        """, (CITY_ID,))
        db_inner_ids = {str(row[0]) for row in cur2.fetchall()}
        sold_ids = db_inner_ids - seen_inner_ids
        # Safety threshold: only mark as sold if we scraped at least 60% of what's in DB.
        # A partial scrape (fewer pages than expected) must not mass-deactivate apartments.
        coverage = len(seen_inner_ids) / max(len(db_inner_ids), 1)
        if coverage < 0.6:
            print(f'\n⚠️  Охват {coverage:.0%} ({len(seen_inner_ids)} из {len(db_inner_ids)}) — '
                  f'пометка проданных пропущена (порог 60%)')
        elif sold_ids:
            cur2.execute("""
                UPDATE properties
                SET is_active = FALSE, status = 'sold', updated_at = NOW()
                WHERE city_id = %s AND inner_id = ANY(%s::varchar[])
            """, (CITY_ID, list(sold_ids)))
            sold_count = cur2.rowcount
            conn2.commit()
    ...
    geo_updated = geocode_from_apartments(geo_cur, geo_conn)
    geo_conn.close()
