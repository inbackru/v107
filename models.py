...
class User(UserMixin, db.Model):
    ...
    social_avatar = db.Column(db.String(500), nullable=True)
...
class PhoneChangeRequest(db.Model):
    ...
    @classmethod
    def create_request(cls, user_id, new_phone, account_type='user'):
    ...
    @classmethod
    def can_request_new_code(cls, user_id, account_type='user', rate_limit_seconds=60):
        from datetime import timedelta
        time_threshold = datetime.utcnow() - timedelta(seconds=rate_limit_seconds)
...
class EmailChangeRequest(db.Model):
    ...
    @classmethod
    def create_request(cls, user_id, new_email, account_type='user'):
    ...
    @classmethod
    def can_request_new_token(cls, user_id, account_type='user', rate_limit_seconds=60):
        from datetime import timedelta
        time_threshold = datetime.utcnow() - timedelta(seconds=rate_limit_seconds)
        recent_request = cls.query.filter(
            cls.user_id == user_id,
...
class Manager(UserMixin, db.Model):
    ...
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    last_login = db.Column(db.DateTime, nullable=True)
    show_on_index = db.Column(db.Boolean, default=False)
    telegram_id = db.Column(db.String(50), nullable=True)
    
    notify_email = db.Column(db.Boolean, default=True)
    notify_telegram = db.Column(db.Boolean, default=True)
    notify_new_leads = db.Column(db.Boolean, default=True)
    notify_new_deals = db.Column(db.Boolean, default=True)
    notify_task_reminders = db.Column(db.Boolean, default=True)
    notify_overdue_tasks = db.Column(db.Boolean, default=True)
    notify_presentation_views = db.Column(db.Boolean, default=True)
    ...
    chat_accept = db.Column(db.Boolean, default=True)

    # Telegram link-code (one-time 6-digit code for linking manager account to bot)
    tg_link_code = db.Column(db.String(10), nullable=True)
    tg_link_code_expires = db.Column(db.DateTime, nullable=True)
...
class Document(db.Model):
    ...
    deal = db.relationship('Deal', backref='documents')
...
class ResidentialComplex(db.Model):
    ...
    distance_to_center = db.Column(db.Float, nullable=True)  # Distance to city center in km (auto-updated)
    ...
    finishing_type = db.Column(db.String(100), nullable=True)  # Отделка (предчистовая, чистовая и т.д.)
    parking_type = db.Column(db.String(100), nullable=True)    # Тип парковки (подземная, гостевая и т.д.)
    floors_min = db.Column(db.Integer, nullable=True)          # Минимальная этажность корпусов
    floors_max = db.Column(db.Integer, nullable=True)          # Максимальная этажность корпусов
    logo_url = db.Column(db.String(400), nullable=True)        # Логотип застройщика/ЖК
    finishing_variants = db.Column(db.Text, nullable=True)     # JSON: варианты отделки от застройщика
...
class ComplexReview(db.Model):
    __tablename__ = 'complex_reviews'
    __table_args__ = {'extend_existing': True}
    ...
    moderated_by = db.Column(db.Integer, nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    is_resident = db.Column(db.Boolean, default=False)
    purchase_year = db.Column(db.Integer, nullable=True)

    residential_complex = db.relationship('ResidentialComplex', backref=db.backref('reviews', lazy='dynamic'))
    user = db.relationship('User', backref=db.backref('complex_reviews', lazy='dynamic'))

    def __repr__(self):
        return f'<ComplexReview {self.author_name} for Complex #{self.residential_complex_id}>'
...
class PromoBanner(db.Model):
    ...
    link_text = db.Column(db.String(100), default='Подробнее')
    is_active = db.Column(db.Boolean, default=True, nullable=False)
    bg_color = db.Column(db.String(20), default='#EEF2FF')
    sort_order = db.Column(db.Integer, default=0, nullable=False)
    placement = db.Column(db.String(20), default='header', nullable=False)
    deadline_text = db.Column(db.String(100), nullable=True)
    large_text = db.Column(db.String(50), nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)

    def __repr__(self):
        return f'<PromoBanner {self.title}>'
...
class UserBalance(db.Model):
    ...
    # Balance details
    balance = db.Column(db.Numeric(15, 2), default=Decimal('0.00'), nullable=False)  # Доступно для вывода
    pending_balance = db.Column(db.Numeric(15, 2), default=Decimal('0.00'), nullable=False)  # В обработке (заявки на вывод)
    ...
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)
    last_updated = db.Column(db.DateTime, nullable=True)  # Последняя транзакция
    
    # Relationship
    user = db.relationship('User', backref=db.backref('user_balance', uselist=False))  # One-to-one
    
    def __repr__(self):
        return f'<UserBalance User #{self.user_id}: {self.balance}₽ available, {self.pending_balance}₽ pending>'
...
class PriceHistory(db.Model):
    ...
    complex_id = db.Column(db.Integer, db.ForeignKey('residential_complexes.id'), nullable=True, index=True)
    record_type = db.Column(db.String(20), nullable=False, default='property')
    price = db.Column(db.BigInteger, nullable=True)
    price_per_sqm = db.Column(db.Integer, nullable=True)
    avg_price = db.Column(db.BigInteger, nullable=True)
    avg_price_per_sqm = db.Column(db.Integer, nullable=True)
    min_price = db.Column(db.BigInteger, nullable=True)
    max_price = db.Column(db.BigInteger, nullable=True)
    properties_count = db.Column(db.Integer, nullable=True)
    price_change_percent = db.Column(db.Float, nullable=True)
    recorded_at = db.Column(db.DateTime, default=datetime.utcnow, index=True)
    month = db.Column(db.Integer, nullable=True)
    year = db.Column(db.Integer, nullable=True)

    property = db.relationship('Property', backref=db.backref('price_history', lazy='dynamic'))
    complex = db.relationship('ResidentialComplex', backref=db.backref('price_history', lazy='dynamic'))

    def __repr__(self):
        return f'<PriceHistory {self.record_type} {self.recorded_at}>'


class PushSubscription(db.Model):
    __tablename__ = 'push_subscriptions'
    id = db.Column(db.Integer, primary_key=True)
    endpoint = db.Column(db.Text, unique=True, nullable=False, index=True)
    p256dh = db.Column(db.Text, nullable=True)
    auth_key = db.Column(db.Text, nullable=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True, index=True)
    manager_id = db.Column(db.Integer, db.ForeignKey('managers.id'), nullable=True, index=True)
    guest_session = db.Column(db.String(100), nullable=True, index=True)
    user_agent = db.Column(db.String(500), nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    is_active = db.Column(db.Boolean, default=True)

    user = db.relationship('User', backref=db.backref('push_subscriptions', lazy='dynamic'))
    manager = db.relationship('Manager', backref=db.backref('push_subscriptions', lazy='dynamic'))

    def __repr__(self):
        return f'<PushSubscription {self.id} user={self.user_id} guest={self.guest_session}>'


class ChatRoom(db.Model):
    """Chat room between a user and a manager"""
    __tablename__ = 'chat_rooms'
    __table_args__ = {"extend_existing": True}

    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True, index=True)
    guest_session = db.Column(db.String(120), nullable=True, index=True)
    ...
    manager = db.relationship('Manager', backref=db.backref('managed_chat_rooms', lazy='dynamic'))
    messages = db.relationship('ChatMessage', backref='room', lazy='dynamic', order_by='ChatMessage.created_at')

    def unread_for_manager(self):
        from sqlalchemy import or_
        return self.messages.filter(
            ChatMessage.is_read_manager == False,
            or_(ChatMessage.sender_type == 'user', ChatMessage.sender_type == 'guest')
        ).count()

    def unread_for_user(self):
        return self.messages.filter_by(is_read_user=False, sender_type='manager').count()
...
class ChatMessage(db.Model):
    ...
    sender_type = db.Column(db.String(20), nullable=False)  # 'user' | 'manager' | 'guest'
    sender_id = db.Column(db.Integer, nullable=True)
    text = db.Column(db.Text, nullable=True)
    file_url = db.Column(db.String(500), nullable=True)
    file_name = db.Column(db.String(255), nullable=True)
    file_type = db.Column(db.String(50), nullable=True)   # 'image' | 'document'
    is_read_manager = db.Column(db.Boolean, default=False)
    ...
    def to_dict(self):
        return {
            ...
            'is_read_manager': self.is_read_manager,
            'is_read_user': self.is_read_user,
            'created_at': (self.created_at + __import__('datetime').timedelta(hours=3)).strftime('%H:%M'),
            'created_date': (self.created_at + __import__('datetime').timedelta(hours=3)).strftime('%d.%m.%Y %H:%M'),
        }
...
class PushBroadcast(db.Model):
    """History of push notification broadcasts sent from admin panel."""
    __tablename__ = 'push_broadcasts'
    id         = db.Column(db.Integer, primary_key=True)
    title      = db.Column(db.String(200), nullable=False)
    body       = db.Column(db.Text, nullable=False)
    url        = db.Column(db.String(500), default='/')
    target     = db.Column(db.String(50), default='all')  # all / all_users / all_managers / guests
    ...
    def __repr__(self):
        return f'<PushBroadcast {self.id} "{self.title[:30]}" → {self.delivered} delivered>'


class ChatSettings(db.Model):
    """Global chat configuration (key-value store)"""
    ...
    def get(key, default=None):
        try:
            s = ChatSettings.query.filter_by(key=key).first()
    ...
    def set_value(key, value):
        ...
        if s:
            ...
            s = ChatSettings(key=key, value=value)
            db.session.add(s)

    def __repr__(self):
        return f'<ChatSettings {self.key}>'


class TelegramSupportSession(db.Model):
    """Live Telegram support chat session (LiveGram pattern)"""
    __tablename__ = 'telegram_support_sessions'
    __table_args__ = {"extend_existing": True}

    id = db.Column(db.Integer, primary_key=True)
    user_tg_id = db.Column(db.String(50), nullable=False, index=True)
    user_name = db.Column(db.String(120), nullable=True)
    user_username = db.Column(db.String(80), nullable=True)
    manager_tg_id = db.Column(db.String(50), nullable=True, index=True)
    manager_db_id = db.Column(db.Integer, db.ForeignKey('managers.id'), nullable=True)
    status = db.Column(db.String(20), default='queue', index=True)  # queue/active/closed
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    last_msg_at = db.Column(db.DateTime, default=datetime.utcnow)
    closed_at = db.Column(db.DateTime, nullable=True)
    queue_notified_at = db.Column(db.DateTime, nullable=True)

    manager = db.relationship('Manager', backref=db.backref('tg_support_sessions', lazy='dynamic'))
    messages = db.relationship('TelegramSupportMessage', backref='session', lazy='dynamic',
                               order_by='TelegramSupportMessage.created_at')

    def __repr__(self):
        return f'<TgSupportSession {self.id} user={self.user_tg_id} status={self.status}>'


class TelegramSupportMessage(db.Model):
    """Tracks individual messages for reply-detection in Telegram support"""
    __tablename__ = 'telegram_support_messages'
    __table_args__ = {"extend_existing": True}

    id = db.Column(db.Integer, primary_key=True)
    session_id = db.Column(db.Integer, db.ForeignKey('telegram_support_sessions.id'), nullable=False, index=True)
    direction = db.Column(db.String(20), nullable=False)  # user_to_mgr / mgr_to_user
    text = db.Column(db.Text, nullable=True)
    forwarded_msg_id = db.Column(db.Integer, nullable=True)  # msg_id on manager side (for reply detection)
    manager_tg_id = db.Column(db.String(50), nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

    def __repr__(self):
        return f'<TgSupportMsg {self.id} dir={self.direction}>'
