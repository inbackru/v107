...
function syncUrlFiltersToMapFilters() {
    ...
    // Also handle singular 'developer' param (from hero search navigation)
    const singleDev = url.get('developer');
    if (singleDev) {
        if (!window.mapFilters.developers) window.mapFilters.developers = [];
        if (!window.mapFilters.developers.includes(singleDev)) {
            window.mapFilters.developers.push(singleDev);
        }
    }
...
function createAggregatedYandexMarker(point) {
    ...
    const marker = new ymaps.Placemark(
        [point.lat, point.lng],
        {
            balloonContent: (() => {
                ...
                const complexLink = point.complex_slug
                    ? `<a href="/${cSlug}/novostrojki/${point.complex_slug}" style="color:#0088CC;text-decoration:underline;font-weight:600;">${point.complex_name || 'Объект'}${point.building_name ? ' — ' + point.building_name : ''}</a>`
                    : `${point.complex_name || 'Объект'}${point.building_name ? ' — ' + point.building_name : ''}`;
                return `${complexLink}<br>${count} квартир от ${priceText}₽`;
            })(),
    ...
    marker.events.add('click', function() {
        ...
        fetch(`/api/map-properties/at-point?${urlFilters.toString()}`, { credentials: 'same-origin' })
            .then(r => r.json())
            .then(data => {
                ...
                if (isMobileDevice()) {
                    ...
                    updateDesktopPropertiesPanel(data.properties, panelTitle);
                }
            })
            .catch(err => console.error('❌ at-point fetch error:', err));
    });
            ...
            fetch(`/api/map-properties/aggregated?${urlFilters.toString()}`, { credentials: 'same-origin' })
                ...
                .then(data => {
                    ...
                    function loadByViewport() {
                        ...
                        fetch(`/api/map-properties/aggregated?${p.toString()}`, { credentials: 'same-origin' })
                            .then(r => r.json())
                            .then(d => { if (d.success && d.points) renderAggregatedPoints(d.points, 'viewport'); isLoading = false; })
                            .catch(() => { isLoading = false; });
                    }
...
function openPropertyBottomSheet(properties) {
    ...
    if (countEl) {
        if (total === 0)       countEl.textContent = '0 объектов';
        else if (total === 1)  countEl.textContent = '1 объект';
        else                   countEl.textContent = `${total} объектов`;
    }
    ...
    function renderBatch() {
        ...
        for (let i = rendered; i < end; i++) {
            try {
                ...
                console.error('❌ Card creation error', properties[i] && properties[i].id, e);
            }
        }
        rendered = end;

        // Remove old "load more" sentinel if any
        ...
        if (rendered < total) {
            ...
            sentinel.className = 'bs-load-more text-center py-3';
            sentinel.innerHTML = `<button class="px-4 py-2 text-sm font-medium text-[#0088CC] border border-[#0088CC] rounded-full hover:bg-blue-50 transition-colors">
                Показать ещё ${remaining}
            </button>`;
            sentinel.querySelector('button').addEventListener('click', renderBatch);
            container.appendChild(sentinel);
        }
        console.log(`✅ Rendered ${rendered}/${total} cards`);
    }
    ...
    container.style.maxHeight = `calc(${sheetH} - 60px)`;
...
function createDesktopPropertyCard(property) {
    ...
    card.innerHTML = `
        ...
        <div class="relative image-slider-container bg-gray-200 overflow-hidden" style="height: 180px; max-height: 180px;" onmouseenter="startImageSlider(this)" onmouseleave="stopImageSlider(this)" onclick="nextSliderImage(this)">
            <img alt="${property.residential_complex}" class="w-full h-full object-cover object-center main-image cursor-pointer transition-all duration-300" src="${firstImage}" style="height: 180px; max-height: 180px;" onerror="this.src='/static/images/no-photo.svg';"/>
        ...
        <div class="p-3 flex-1 flex flex-col">
            ...
            <div class="flex-1">
                ...
                ${property.address ? `<p class="text-xs text-gray-400 mt-0.5 line-clamp-1">📍 ${property.address}</p>` : (developer ? `<p class="text-xs text-gray-500 mt-0.5">${developer}</p>` : '')}
            </div>
        </div>
    `;
...
function updateActiveFiltersDisplay() {
    ...
    if (Array.isArray(mapFilters.districts) && mapFilters.districts.length > 0) {
        pills.push(`<span class="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-medium">
            Районов: ${mapFilters.districts.length}
        </span>`);
    }
...
function updateDesktopPropertiesPanel(properties, complexName) {
    ...
    // ✅ For filtered results use lazy infinite scroll; for ЖК clusters render all
    if (complexName === 'Отфильтрованные объекты' || complexName === 'Свойства в области') {
        ...
        // Specific ЖК cluster — render all at once (usually < 30 items)
        properties.forEach(property => {
            const card = createDesktopPropertyCard(property);
            card.addEventListener('mouseenter', function() {
                if (!fullscreenMapInstance) return;
                fullscreenMapInstance.geoObjects.each(placemark => {
                    ...
                    if (markerCoords && markerCoords[0] === property.coordinates?.lat && markerCoords[1] === property.coordinates?.lng) {
