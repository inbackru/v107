    ...
    complexes.forEach((complex, index) => {
        ...
        images.forEach((img, i) => {
            ...
            slidesHtml += `<div class="carousel-slide absolute inset-0 w-full h-full transition-opacity duration-300 ${isFirst ? 'opacity-100 z-10' : 'opacity-0 z-0'}" data-slide="${i}">
                <img src="${img}" referrerpolicy="no-referrer" class="w-full h-full object-cover bg-gray-200" alt="${complex.name}" onerror="this.src='/static/images/no-photo.svg'">
            </div>`;
        });
...
function createEnhancedComplexMarker(complexes) {
    ...
    const iconLayout = ymaps.templateLayoutFactory.createClass(
        '<div class="ymap-complex-marker" style="position: relative; cursor: pointer; pointer-events: all; display: flex; justify-content: center;">' +
    ...
    placemark.properties.set({
        image: complexes[0].main_image || '/static/images/no-photo.svg',
    ...
    placemark.events.add('click', function(e) {
        ...
        e.preventDefault();
        try {
            // Always pan/zoom to the clicked complex
            if (fullscreenComplexesMapInstance) {
                const currentZoom = fullscreenComplexesMapInstance.getZoom();
                const targetZoom = Math.max(currentZoom, 14);
                fullscreenComplexesMapInstance.setCenter(coords, targetZoom, { duration: 400, checkZoomRange: true });
            }
            if (isMobileDevice()) {
