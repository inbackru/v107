...
class ComparisonManager {
    ...
    showNotification(message, type = 'success') {
        ...
        // Fallback for when toast.js not yet loaded
        const notification = document.createElement('div');
        notification.style.cssText = `
            position: fixed; bottom: 80px; left: 50%; transform: translateX(-50%) translateY(20px);
            background: ${type === 'success' ? '#059669' : type === 'warning' ? '#D97706' : '#0088CC'};
            color: white; padding: 12px 20px; border-radius: 12px; font-size: 14px; font-weight: 500;
            box-shadow: 0 8px 25px rgba(0,0,0,0.2); z-index: 99999; opacity: 0;
