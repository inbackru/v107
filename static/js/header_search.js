...
class SmartSearch {
    ...
    initHeroSearch() {
        if (this.heroSearchInput) {
            ...
            // Check for mobile search input which might be different in HTML
            const mobileSearchInput = document.getElementById('index-modal-search-input') || document.getElementById('hero-search-mobile');
            const mobileDropdown = document.getElementById('indexSearchResults') || document.getElementById('hero-searchSuggestions-mobile');
            ...
            if (mobileSearchInput && mobileDropdown) {
                console.log('Mobile/Modal search input found, integrating with SmartSearch');
                this.initSearchInput(mobileSearchInput, mobileDropdown);
            }
    ...
    executeSearch(query) {
        if (query) {
            // If we're on a city properties page, apply smart filters in place
            const cityPropsMatch = window.location.pathname.match(/^\/([^/]+)\/novostrojki/);
            if (cityPropsMatch && this.propertySearchInput) {
                this.performSmartSearchOnPage(query);
            } else if (window.location.pathname === '/properties' && this.propertySearchInput) {
                this.performSmartSearchOnPage(query);
            } else {
                // Redirect to city properties page
...
SmartSearch.prototype.renderModernSuggestions = function(suggestions, dropdown) {
    ...
    suggestions.forEach(suggestion => {
        const label = suggestion.text || suggestion.title || '';
        const firstLetter = label.replace(/^ЖК\s*/i, '').charAt(0).toUpperCase() || '?';
        const letterAvatar = `<span style="font-size:14px;font-weight:700;color:#0088CC;">${firstLetter}</span>`;
        const iconHtml = suggestion.image_url
            ? `<div style="position:relative;width:100%;height:100%;overflow:hidden;border-radius:50%;display:flex;align-items:center;justify-content:center;">
                   ${letterAvatar}
                   <img src="${suggestion.image_url}" referrerpolicy="no-referrer" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;border-radius:50%;" onerror="this.remove()">
               </div>`
            : (suggestion.type === 'complex' ? letterAvatar : `<i class="${iconMap[suggestion.type] || 'fas fa-search'} text-sm"></i>`);
        const iconWrapClass = suggestion.image_url
            ? 'w-8 h-8 rounded-full overflow-hidden mr-3 flex-shrink-0 flex items-center justify-center bg-blue-50'
            : 'w-8 h-8 bg-blue-50 rounded-full flex items-center justify-center mr-3 flex-shrink-0';
        html += `
    ...
    dropdown.querySelectorAll('.suggestion-item').forEach(item => {
        item.addEventListener('click', () => {
            ...
            console.log('🔍 Suggestion clicked:', { type, value, url: rawUrl });
            
            // Guard against literal "undefined" attribute value
            const url = (rawUrl && rawUrl !== 'undefined' && rawUrl !== 'null') ? rawUrl : null;
            
            if (url) {
                window.location.href = url;
            } else if (value) {
                // Fallback: build city-aware search URL from suggestion text
                const citySlug = window.currentCitySlug || document.documentElement.dataset.citySlug || 'krasnodar';
                window.location.href = '/' + citySlug + '/novostrojki?search=' + encodeURIComponent(value);
            }
        });
    });
};