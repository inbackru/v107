...
class SmartAutocomplete {
    ...
    async searchSuggestions(query) {
        ...
        try {
            ...
            const _url = new URL(this.options.apiUrl, window.location.origin);
    ...
    displaySuggestions(suggestions) {
        ...
        const html = suggestions.map((suggestion, index) => {
            ...
            const iconHtml = suggestion.image_url
                ? `<div style="position:relative;width:32px;height:32px;border-radius:50%;background:#e0f0fa;display:flex;align-items:center;justify-content:center;overflow:hidden;font-size:14px;font-weight:700;color:#0088CC;">${firstLetter}<img src="${suggestion.image_url}" referrerpolicy="no-referrer" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;" onerror="this.remove()"></div>`
                : (suggestion.type === 'complex'
                    ? `<span style="display:flex;width:32px;height:32px;border-radius:50%;background:#e0f0fa;align-items:center;justify-content:center;font-size:14px;font-weight:700;color:#0088CC;">${firstLetter}</span>`
                    : `<i class="${suggestion.icon || 'fas fa-search'}"></i>`);
            return `
            <div class="suggestion-item ${index === this.selectedIndex ? 'selected' : ''}" data-index="${index}">
                <div class="suggestion-icon" style="${suggestion.image_url || suggestion.type === 'complex' ? 'background:none;border:none;' : ''}">
                    ${iconHtml}
                </div>
