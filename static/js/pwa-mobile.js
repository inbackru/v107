...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    /* Geo request only on mobile after 2.5 s */
    if (isMobile && navigator.geolocation) {
      _withQueue(requestGeoPermission, 2500);
    }

    /* PWA standalone: init pull-to-refresh */
    if (isStandalone) initPullToRefresh();

    /* Smart push permission — only after real engagement (60s + 2 property views) */
    _trackPageEngagement();
    var _pushVisits = parseInt(ls('inback_pv_count') || '0');
    if (_canAskPush() && _pushVisits >= 2) {
      _withQueue(function() { _showPushCard(); }, 60000);
    }
